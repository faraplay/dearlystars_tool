const MAGIC_30: u32 = 0x72636E65;
const MAGIC_34: u32 = 0x6A624F79;
...
pub fn encrypt_arm9(gamecode: u32, data: &[u8; 0x4000]) -> [u8; 0x4000] {
    let mut p: [u32; 0x1000] = data
        .as_chunks::<4>()
        .0
        .iter()
        ...
        .try_into()
        .unwrap();
    if p[0] != 0xE7FFDEFFu32 || p[1] != 0xE7FFDEFFu32 {
        panic!("Data to encrypt does not start with FF DE FF E7 FF DE FF E7!");
    }
    let mut offset = 2;
    let (mut card_hash, mut arg2) = init1(gamecode);
    arg2[1] <<= 1;
    arg2[2] >>= 1;
    init2(&mut card_hash, &mut arg2);

    let mut size = 0x800 - 8;
    while size > 0 {
        let [p0, p1] = &mut p[offset..offset + 2] else {
            unreachable!()
        };
        encrypt(&card_hash, p1, p0);
        offset += 2;
        size -= 8;
    }
    ...
    let [p0, p1] = &mut p[0..2] else {
        unreachable!()
    };
    *p0 = MAGIC_30;
    *p1 = MAGIC_34;
    encrypt(&card_hash, p1, p0);
    card_hash = init1(gamecode).0;
    encrypt(&card_hash, p1, p0);
...
pub fn decrypt_arm9(gamecode: u32, data: &[u8; 0x4000]) -> [u8; 0x4000] {
    let mut p: [u32; 0x1000] = data
        .as_chunks::<4>()
        .0
        .iter()
    ...
    decrypt(&card_hash, p1, p0);
    arg2[1] <<= 1;
    arg2[2] >>= 1;
    init2(&mut card_hash, &mut arg2);
    decrypt(&card_hash, p1, p0);

    if *p0 != MAGIC_30 || *p1 != MAGIC_34 {
        panic!("Decrypted data does not start with 65 6E 63 72 79 4F 62 6A!");
    }

    *p0 = 0xE7FFDEFFu32;
    *p1 = 0xE7FFDEFFu32;
    let mut size: u32 = 0x800 - 8;
    let mut offset = 2;
    while size > 0 {
        let [p0, p1] = &mut p[offset..offset + 2] else {
            unreachable!()
...
fn init1(gamecode: u32) -> ([u32; 0x412], [u32; 3]) {
    let mut card_hash: [u32; 0x412] = ENCR_DATA
        .as_chunks::<4>()
        .0
        .iter()
        .map(|chunk| u32::from_le_bytes(*chunk))
        ...
        .try_into()
        .unwrap();
    let mut arg2 = [gamecode, gamecode >> 1, gamecode << 1];
    init2(&mut card_hash, &mut arg2);
    init2(&mut card_hash, &mut arg2);
    (card_hash, arg2)
}

fn init2(magic: &mut [u32; 0x412], a: &mut [u32; 3]) {
    let [a0, a1, a2] = a;
    encrypt(magic, a2, a1);
    encrypt(magic, a1, a0);
    let arg1 = [a0.to_le_bytes(), a1.to_le_bytes()]
        .concat()
        .try_into()
        .unwrap();
    update_hashtable(magic, &arg1);
}

fn encrypt(magic: &[u32; 0x412], arg1: &mut u32, arg2: &mut u32) {
    let mut a = *arg1;
    let mut b = *arg2;
    let mut c;
    for i in 0..16 {
...
fn decrypt(magic: &[u32; 0x412], arg1: &mut u32, arg2: &mut u32) {
    let mut a = *arg1;
    let mut b = *arg2;
    let mut c;
    for i in (2..18).rev() {
...
fn lookup(magic: &[u32; 0x412], v: u32) -> u32 {
    let a = magic[(((v >> 24) & 0xFF) + 18 + 0) as usize];
    let b = magic[(((v >> 16) & 0xFF) + 18 + 256) as usize];
    let c = magic[(((v >> 8) & 0xFF) + 18 + 512) as usize];
    let d = magic[(((v >> 0) & 0xFF) + 18 + 768) as usize];
    d.wrapping_add(c ^ (b.wrapping_add(a)))
}

fn update_hashtable(magic: &mut [u32; 0x412], arg1: &[u8; 8]) {
    for j in 0..18 {
        let mut r3: u32 = 0;
        for i in 0..4 {
            r3 <<= 8;
            r3 |= arg1[(j * 4 + i) & 7] as u32;
        }
        magic[j] ^= r3;
    }
    ...
    for i in (0..18).step_by(2) {
        encrypt(magic, &mut tmp1, &mut tmp2);
        magic[i + 0] = tmp1;
        magic[i + 1] = tmp2;
    }
