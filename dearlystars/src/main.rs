...
#[derive(Subcommand)]
enum Commands {
    /// Decompress a file using the blz algorithm.
    ///
    /// This is often used for executables in a .nds file, such as arm9.bin
    DecompressBlz {
    ...
    /// Compress a file using the blz algorithm.
    ///
    /// This is often used for executables in a .nds file, such as arm9.bin
    CompressBlz {
        ...
        /// The output file.
        out_bin_file: PathBuf,
        /// The size of the region to keep uncompressed. Default is 0.
        ///
        /// You should probably set this to 16384 if you are compressing arm9.bin.
...
fn main() {
    let cli = Cli::parse();
    match &cli.command {
        Commands::DecompressBlz {
            in_bin_file,
            out_bin_file,
        } => {
            let in_data = std::fs::read(in_bin_file).expect("Error reading input bin file!");
            let out_data =
                ndstool::blz_decompress(&in_data).expect("Error decompressing bin file!");
            std::fs::write(out_bin_file, &out_data).expect("Error writing data to output file!");
            println!("Decompressed file.")
        }
        Commands::CompressBlz {
            in_bin_file,
            out_bin_file,
